...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (autoOpenId) {
        ...
        if (content) {
            const block = content.closest('.virtual-archive-block') || content.closest('.file-item');
            if (block) {
                // Try finding the trigger: either the header with onclick (Rating) or the button in file-actions (Files)
                const trigger = block.querySelector('.files-card-header') || block.querySelector('.file-manage-btn');
                if (trigger) {
                    console.log("✅ Trigger found, simulating click.");
                    trigger.click();
                }
            }
        }
