...
def view(request: Request, path: str, db: DB, user: CurrentUser, background_tasks: BackgroundTasks, edit: bool = False):
    ...
    if meta_db and meta_db.is_archived and has_real_files:
        logger.warning(f"Incohérence : {target.name} marqué archivé mais contient des fichiers réels. Correction...")
        meta_db.is_archived = False
        db.commit()

    detected_files = []
    archives_virtual_data = []
    
    if meta_db and meta_db.is_archived and meta_db.archived_files_json and not has_real_files:
        archived_data = json.loads(meta_db.archived_files_json)
        for f in archived_data:
            detected_files.append({"filename": f["rel_path"], "is_folder": False, "size_str": format_size(int(f.get("size_mb", 0) * 1024 * 1024)), "url": "#archived", "is_pinned": False, "is_archived": True, "archive_name": f.get("archive", "")})
        temp_map = {}
        for f in archived_data:
            a_name = f.get("archive", "unknown")
            if a_name not in temp_map: temp_map[a_name] = []
            temp_map[a_name].append(f)
        for a_name, f_list in temp_map.items():
            archives_virtual_data.append({"name": a_name, "path": None, "files": LibraryService.build_virtual_tree(f_list), "has_index": True, "on_disk": False})
    else:
        detected_files = physical_files
        if target.exists():
            for f in target.iterdir():
                if f.is_file() and f.suffix.lower() in ['.iso', '.zip']:
                    archives_virtual_data.append({"name": f.name, "path": normalize_path(f.absolute()), "files": None, "has_index": False, "on_disk": True})
